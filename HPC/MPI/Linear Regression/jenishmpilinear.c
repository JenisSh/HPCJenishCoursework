...
double rms_error (double m, double c)
{
   ...
   for (j = 0; j < n_data; j++)
   {
      error_sum += residual_error (data[j].x, data[j].y, m, c);
   }
...
int main () {
   ...
   while (!minimum_found)
   {
      ...
      if (rank != 0)
      {
         ...
         dm[j] = bm + (om[j] * step);
         dc[j] = bc + (oc[j] * step);
         pError = rms_error (dm[j], dc[j]);

         MPI_Send (&pError, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
         MPI_Send (&dm[j], 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
         MPI_Send (&dc[j], 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
      ...
      else
      {
         for (j = 1; j < size; j++)
         {
            MPI_Recv (&pError, 1, MPI_DOUBLE, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv (&dm[j-1], 1, MPI_DOUBLE, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv (&dc[j-1], 1, MPI_DOUBLE, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
         ...
         for (j = 1; j < size; j++)
         {
            MPI_Send (&bm, 1, MPI_DOUBLE, j, 0, MPI_COMM_WORLD);
            MPI_Send (&bc, 1, MPI_DOUBLE, j, 0, MPI_COMM_WORLD);
            MPI_Send (&minimum_found, 1, MPI_INT, j, 0, MPI_COMM_WORLD);
         }
      }
   }
