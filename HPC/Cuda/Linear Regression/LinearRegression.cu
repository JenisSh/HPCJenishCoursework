...
double residual_error(double j, double k, double m, double c) {
  double e = (m * j) + c - k;
  return e * e;
}
__device__ double d_residual_error(double j, double k, double m, double c) {
  double e = (m * j) + c - k;
  return e * e;
}
double rms_error(double m, double c) {
  ...
  for(d=0; d<n_data; d++) {
    error_sum += residual_error(data[d].j, data[d].k, m, c);  
  }
...
__global__ void d_rms_error(double *m, double *c,double *error_sum_arr,point_p *d_data) {
  int d = threadIdx.x + blockIdx.x *blockDim.x;
error_sum_arr[d] = d_residual_error(d_data[d].j,d_data[d].k, *m, *c);
}
...
d_rms_error <<<100,10>>>(&d_dm[d],&d_dc[d],d_error_sum_arr,d_data);
...
e[d] =sqrt(error_sum_mean);

if(e[d] < best_error){
best_error = e[d];
error_sum_total +=h_error_sum_arr[d];
}
error_sum_mean = error_sum_total /n_data;
e[d] =  sqrt(error_sum_mean); 
